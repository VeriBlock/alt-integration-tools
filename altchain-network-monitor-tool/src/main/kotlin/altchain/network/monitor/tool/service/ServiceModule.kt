...
fun serviceModule(configuration: Configuration): Module {
    return module {
        ...
        single { PopSubsidiesService() }
        single { NodeCoreService() }
        single { AbfiService(get()) }
        single { VbfiService() }
        single { MetricsService(get(), get()) }
        single { CleanupService(get(), get(), get(), get(), get(), get(), get(), get()) }

        single { AltchainService(get()) }

        single { MonitorService(get(), get(), get(), get(), get(), get(), get(), get(), get(), get(), get(), get(), get(), get(), get()) }
    }
}